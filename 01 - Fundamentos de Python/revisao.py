...
# Parâmetro com Valor Padrão deve ser declarado ao final
# Argumentos Posicionais x Argumentos Nomeados
# Argumentos Posicionais x Argumentos Nomeados (simbolo que obriga que os próximos argumentos sejam nomeados)
# 0 argumentos posicionais + 2 argumentos nomeados
# 1 argumento posicional + 1 argumento nomeado
# Args
# Kwargs
# Função com os 3 Tipos de Argumentos = Argumento Posicional + Args + Kwargs
# Args são passados como TUPLAS e Kwargs são passados como DICIONÁRIO
# Funções são Objetos de Primeira Classe - armazenar a função em uma variável
# Funções são Objetos de Primeira Classe - passar a função como um parâmetro para outra função
# Funções são Objetos de Primeira Classe - retornar a função de uma função
# Criar um Decorator
...
# Descobrindo o Índice em um Loop - utilizar a função que nos retorna a posição atual do índice
# Ordenar Coleções através de Propriedades - criar uma lista de dicionários e ordenar pelas suas chaves
# Ordenar Coleções através de Propriedades - realizar ordenação multinível (utilizar mais de uma chave)
# Ordenar Coleções através de Propriedades - criar uma lista de tuplas e ordenar utilizando o índice de seus items
# Map - aplicar sobre uma lista
# Map - aplicar sobre uma lista de listas
# Filter - filtrar de uma lista apenas os nomes iniciados com 'M'
...
# JSON - transformar um dicionário em uma STRING JSON
...
# Contruir a estrutura completa, de 4 BLOCOS, para tratamento de excessões
# Utilizar a classe genérica que armazena os tipos de erro e imprimir no terminal
# Log -> criar um programa de cálculo de divisão e registrar no log: USUÁRIO CONCLUIU COM SUCESSO + ERRO (usuário digitou uma string) + ERRO (usuário tentou dividir por zero) + MENSAGEM FINAL DO PROGRAMA
...
# Incluir MÉTODOS ESTÁTICOS que agrupam funcionalidades de código
# Herança Simples - passando atributos para a classe superior + tem seus PRÓPRIOS métodos e atributos
# Herança Simples - SOBRESCREVER método da classe pai
# Herança Simples - verificar se uma classe é instância de uma classe pai
# Herança Multinivel - devemos evitá-la, pois pode tornar o código complexo e dificil de interpretar
# Herança Múltipla - criar uma classe com Herança Múltipla
# Herança Múltipla - aplicar o método que descobre qual classe irá ter prioridade quando um método ou propriedade de mesmo nome for invocado
# Métodos Mágicos - método que determina o que deve ser mensurado para se obter a quantidade(comprimento) de uma determinada classe
# Métodos Mágicos - método que cria uma representação do objeto para o programador no console do Python.
# Métodos Mágicos - método que cria uma representação legível no formato de string e serve para exibir o objeto para usuário final
# Métodos Mágicos - método (não mágico) fornece uma lista de todos os métodos mágicos disponíveis em uma classe
